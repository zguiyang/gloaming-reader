...
This is an admin supply workflow, not learner Book Detail behavior. `admin_epub` is the MVP primary supply; `admin_text` remains an internal dev/test/seed fallback only. The diagram and gates below are evidence snapshots; they do not supersede ADR-001, product docs, or the repo.
...
POST /api/admin/works/epub
  ...
  → parse: EPUB metadata, chapters, cover and referenced images;
           XHTML security sanitization, image placeholder tokens, data-p paragraph anchors
...
| Admin EPUB workflow          | `apps/backend/src/modules/works/route.ts`, `apps/web/features/admin/works/works-api.ts`; `apps/backend/src/modules/epub-ingest/` (`parser.ts`, `clean.ts`, `chapters.ts`, `opf/parse.ts`, `archive/limits.ts`); `apps/backend/src/modules/content-parser/service.ts`; `apps/backend/src/modules/metadata-fill/service.ts`; `apps/backend/src/modules/metadata-enrich/service.ts`; `apps/backend/src/jobs/metadata-enrich.ts`; `apps/backend/src/modules/content-assets/service.ts` |
| Workflow status / jobs       | `apps/backend/src/lib/workflow.ts`, `apps/backend/src/lib/workflow-policy.ts`; `apps/backend/src/jobs/content-parse.ts`, `apps/backend/src/jobs/work-metadata-fill.ts`, `apps/backend/src/jobs/metadata-enrich.ts` (step/status contracts: Shared DTOs → `works.ts`)                                                                                                                                                                                                               |
| TextStack reference          | https://github.com/mrviduus/textstack (`BookDetailPage`, `BookDetailHero`, `BookDetail` type)                                                                                                                                                                                                                                                                                                                                                                                      |

**Revision (post baseline `d870d4f`):** Admin EPUB workflow snapshot updated — publish requires ready default US for synth parts; auto workflow/TTS flags remain off (`workflow-policy.ts`). Source index aligned to current epub-ingest modules and metadata-enrich job/service paths (evidence only).
